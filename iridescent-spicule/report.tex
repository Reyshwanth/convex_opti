...
\begin{itemize}
    ...
    \item \textbf{Epigraph Based SOCP Relaxation:} Modified SDP with linear and convex relaxations.
    \item \textbf{Robust Huber Loss via Epigraph:} Outlier rejection with smooth, convex and non-integer based loss function.
\end{itemize}
...
\subsubsection{SOCP with Epigraph}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{results_socp-epigraph.png}
    \caption{SOCP with Epigraph Relaxation}
    \label{fig:socp_epigraph}
\end{figure}

The SOCP method with epigraph formulation achieves excellent accuracy by solving a globally optimal convex relaxation. Epigraph relaxation and LMI constraints reduce the solve time.

\subsubsection{Robust Huber Loss via Epigraph}

The robust Huber loss achieves comparable results to MIQP with a lower computation time. However, the results are dependent on the hyperparameter ($\delta$) which needs more tuning to get even better results.
...
\midrule
SDP & 0.182 & 3.74 & Poor ($O(n^3)$) \\
ADMM & 0.194 & 0.87 & Excellent ($O(n)$) \\
MIQP & 0.156 & 4.12 & Moderate ($O(n^2)$) \\
SOCP- Epigraph & 0.182 & 1.34 & Moderate ($O(n^{1.5})$) \\
Robust Huber Loss & 0.215 & 2.67 & - \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Epigraph Based SOCP Relaxation}: Comparable to SDP with less computational expense. 
    \item \textbf{Robust Huber Loss}: Explicit outlier handling and faster than MIQP, but challenging to tune the hyperparameter.
\end{itemize}
