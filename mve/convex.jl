...
if problem.status == MathOptInterface.OPTIMAL || problem.status == MathOptInterface.FEASIBLE_POINT
    ...
    # Export data to JSON for Python interactive visualization
    data_export = Dict(
        "beacons" => [collect(B[i]) for i in 1:L],
        "rho" => rho,
        "r_sol" => collect(r_sol),
        "P_sol" => [collect(P_sol[i, :]) for i in 1:3],
        "L" => L
    )
    open("ellipsoid_data.json", "w") do f
        JSON.print(f, data_export)
    end
    println("\nData exported to ellipsoid_data.json for Python visualization")
    println("Run: python3 interactive_plot.py")
else
